...
from nickplutt_com.acm.config import tag_name, domain_name, additional_names


def create_certificate(template=None):
    if not template:
        template = Template()
        template.add_description('This CloudFormation template creates a certificate for nickplutt.com')
